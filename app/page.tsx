...
export default function PropertyPage() {
  const promoToastShown = useRef(false);
  ...
  useEffect(() => {
    ...
    const parsedCheckIn = checkInParam ? parseDate(checkInParam) : null;
    const parsedCheckOut = checkOutParam ? parseDate(checkOutParam) : null;
    const adults = adultParam ? parseInt(adultParam, 10) : 2;
    const children = childParam ? parseInt(childParam, 10) : 0;
    ...
    setDateRange({
      from: parsedCheckIn ?? undefined,
      to: parsedCheckOut ?? undefined,
    });
  ...
  // Clear localStorage on page load, but preserve selectedLanguage, guestSelections, and parsedPromoDetails
  useEffect(() => {
    const selectedLanguage = localStorage.getItem("selectedLanguage");
    const guestSelections = localStorage.getItem("guestSelections");
    const bookingDetails = localStorage.getItem("bookingDetails");
    const parsedPromoDetails = localStorage.getItem("parsedPromoDetails");
    localStorage.clear();
    if (selectedLanguage) {
      localStorage.setItem("selectedLanguage", selectedLanguage);
    }
    if (guestSelections) {
      localStorage.setItem("guestSelections", guestSelections);
    }
    if (parsedPromoDetails) {
      localStorage.setItem("parsedPromoDetails", parsedPromoDetails);
    }
    if (bookingDetails) {
      localStorage.setItem("bookingDetails", bookingDetails);
    }
    // Enforce English on load by setting URL hash if not already set
  ...
  // Populate promo code and booking details from URL on initial load
  // This useEffect must come after dateRange/setDateRange are defined.
  ...
  useEffect(() => {
    ...
    updateBookingDetails({
      checkIn: parsedCheckIn,
      checkOut: parsedCheckOut,
      adults: adultParam ? parseInt(adultParam, 10) : 2,
      children: childParam ? parseInt(childParam, 10) : 0,
  ...
  useEffect(() => {
    ...
    if (promo) {
      ...
      // If no promo param, fall back to any promo details saved in localStorage
      const storedPromoDetails = localStorage.getItem("parsedPromoDetails");
      if (storedPromoDetails) {
        try {
          const parsed: PromoDetails = JSON.parse(storedPromoDetails);
          if (parsed && parsed.PromoCode) {
            setPromoCode(parsed.PromoCode);
            setPromoDetails(parsed);
          }
        } catch (err) {
          console.error("Failed to parse stored promo details", err);
        }
      }
    }
    ...
    if (checkInParam || checkOutParam) {
      updateBookingDetails({
        checkIn: parsedCheckIn,
        checkOut: parsedCheckOut,
        adults: adultParam ? parseInt(adultParam, 10) : bookingDetails.adults,
        children: childParam
          ? parseInt(childParam, 10)
          : bookingDetails.children,
        rooms: roomsParam ? parseInt(roomsParam, 10) : bookingDetails.rooms,
      });

      if (parsedCheckIn && parsedCheckOut) {
        setDateRange({ from: parsedCheckIn, to: parsedCheckOut });
      }
    }
  }, []);

  // Load reservation details from localStorage on mount
  useEffect(() => {
  ...
  type BookingDetailsWithChildAges = typeof bookingDetails & {
    childAges?: number[];
    rooms: number;
  };
  const { bookingDetails, updateBookingDetails, removeRoom } = useBooking();

  // Restore guest selections from localStorage
  useEffect(() => {
    const savedGuests = localStorage.getItem("guestSelections");
    if (savedGuests) {
  ...
  console.log("all promo :", allPromos);

  useEffect(() => {
    const fetchPromoCodes = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/API_IBE/GetPromoCodes.aspx`);
        const data = await res.json();
        console.log("Fetched promo data:", data);
        setAllPromos(data);

        // Show toast if there are promos to display and not already shown
        const showPromos = Array.isArray(data)
          ? data.filter((p) => p.isShowOnIBE)
          : [];
        if (showPromos.length > 0 && !promoToastShown.current) {
          toast.custom(
            (t) => (
              <div
                className={`relative max-w-sm w-full bg-green-50 border border-green-200 rounded-md shadow-md px-3 py-2 flex items-center justify-between space-x-4 ${
                  t.visible ? "animate-enter" : "animate-leave"
                }`}
                style={{ marginLeft: "auto", marginRight: "1.5rem" }}
              >
                <button
                  className="absolute top-1 left-1 text-green-600 hover:text-green-800 text-sm"
                ...
                >
                  ...
                  <span className="text-green-800 font-medium">
                    {showPromos.length} promotion
                    {showPromos.length > 1 ? "s" : ""} available!
                  </span>
                </div>
              </div>
            ),
  ...
  const bookingDetailsWithRooms: BookingDetailsWithChildAges = {
    ...bookingDetails,
    adults:
      typeof bookingDetails.adults === "number" ? bookingDetails.adults : 2,
    children:
      typeof bookingDetails.children === "number" ? bookingDetails.children : 0,
  ...
  useEffect(() => {
    const checkIn =
      ...
      JSON.parse(localStorage.getItem("bookingDetails") || "{}")?.checkIn;
    const checkOut =
      bookingDetails.checkOut ??
      JSON.parse(localStorage.getItem("bookingDetails") || "{}")?.checkOut;
  ...
  useEffect(() => {
    const fetchRoomRates = async () => {
      ...
      if (!bookingDetailsWithRooms.checkIn || !bookingDetailsWithRooms.checkOut)
        return;
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/API_IBE/GetRoomRates.aspx?datefrom=${dateFrom}&dateto=${dateTo}&currency=US$`
        );
        ...
        const totalGuests =
          (bookingDetailsWithRooms.adults || 0) +
          (bookingDetailsWithRooms.children || 0);
        ...
        const roomsWithRates = data
          .filter((room) => {
            ...
            return (
              room.maxavailable >= requiredRooms &&
              maxGuestsPerRoom >= totalGuests / requiredRooms
            );
          })
          .map((room) => {
            ...
            const rate =
              baseRate + (childRate > 0 ? childRate * applicableChildren : 0);
            ...
            return {
              ...
              availableMealPlans: room.availableMealPlans ?? [
                room.mealplan ?? "",
              ],
        ...
        roomsWithRates.sort((a, b) => a.price - b.price);
    ...
    bookingDetailsWithRooms.checkIn,
    bookingDetailsWithRooms.checkOut,
    bookingDetailsWithRooms.adults,
    bookingDetailsWithRooms.children,
  ]);
  ...
  console.log("bookingDetails selectedRooms", bookingDetails.selectedRooms);
  ...
  const handleProceedToBooking = () => {
    ...
    const reservationSummary = {
      selectedRooms: bookingDetails.selectedRooms.map((room) => ({
        ...
        perNightRate: room.price / (bookingDetails.nights || 1),
      })),
      promoCode,
      selectedPackages,
    ...
    const selectedRoomTypes = bookingDetails.selectedRooms
      .map((room) => room.roomName)
      .join(",");
    const packageIDs = selectedPackages.map((pkg) => pkg.PackageID).join(",");
    const promo = promoCode ? `&promo=${encodeURIComponent(promoCode)}` : "";
    router.push(
      `/book?roomTypes=${encodeURIComponent(
        selectedRoomTypes
      )}${promo}&packages=${encodeURIComponent(packageIDs)}`
    );
  };

  // Calculate total price for all selected rooms
  const totalSelectedPrice = bookingDetails.selectedRooms.reduce(
    (total, room) => {
  ...
  const discountValue = (() => {
    ...
    if (
      ...
      const totalRoomPrice = bookingDetails.selectedRooms.reduce(
        (total, room) => {
          return total + room.price * room.quantity;
        },
        0
      ...
      const perNightPrice = totalRoomPrice / bookingDetails.nights;
      return perNightPrice * promoDetails.FreeNights;
    }
  ...
  // On initial mount or booking details change, auto-click "View Available Rooms" only if booking details are incomplete
  useEffect(() => {
    if (
      bookingDetails.checkIn &&
      bookingDetails.checkOut &&
      ...
      return; // Skip clicking if valid booking details exist
    }
  ...
  return (
    <>
      ...
      <Header />
      <HotelNetworkScript propertyId={hotelData?.HotelNetworkID || "1039100"} />
      <div className="container max-w-7xl mx-auto px-4 pt-4">
        {/* Hotel name, score, star category, and Currency Selector */}
        <div className="flex sm:flex-row flex-col justify-between items-start sm:items-center gap-1 sm:gap-2">
          <div>
            <h1 className="text-2xl font-bold" translate="no">
              {hotelData?.HotelName || "Property Name"}
            </h1>
            {hotelData?.Address && (
              <div className="hidden sm:flex items-center text-xs text-muted-foreground mt-1">
                ...
                <span translate="no">{hotelData.Address}</span>
              </div>
            )}
          </div>
          <div className="w-full sm:w-auto flex justify-end mt-0">
            <CurrencySelector />
          </div>
        </div>
      </div>
  ...
  "
            >
              <div className="space-y-2">
                ...
                <Popover>
                  ...
                  <PopoverContent className="p-0" align="start" sideOffset={4}>
                    <div
                      className="w-[370px] sm:w-auto overflow-x-auto sm:overflow-visible [&_.rdrDefinedRangesWrapper]:hidden"
                      translate="no"
                    >
                      <DateRangePicker
                        ranges={[
              ...
              <div className="space-y-2">
                ...
                <GuestSelector
                  key={`${bookingDetails.adults ?? 2}-${
                    bookingDetails.children ?? 0
                  }-${bookingDetails.rooms ?? 1}`}
                  defaultAdults={bookingDetails.adults ?? 2}
                  defaultChildren={bookingDetails.children ?? 0}
                  defaultRooms={bookingDetails.rooms ?? 1}
                  onChange={(adults, children, rooms) => {
                    localStorage.setItem(
                      "guestSelections",
                      JSON.stringify({ adults, children, rooms })
                    );
                    updateBookingDetails({ adults, children, rooms });
                  }}
                />
              </div>
            ...
            <CardContent className="p-4">
              <div className="flex justify-between items-center mb-4">
                ...
                <Button
                  ...
                  className="bg-primary btn-dynamic"
                >
                  Proceed to Booking
                </Button>
              </div>
              ...
              <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end justify-between h-full">
                      <div className="text-right">
                        <div>
                          {formatPrice(
                            convertPrice(room.price * room.quantity)
                          )}
                          /period
                        </div>
                        <div className="text-sm text-muted-foreground">
                ...
                <div className="flex justify-between items-center pt-3 border-t mt-3">
                  <div className="font-medium">
                    Total for {bookingDetails.nights || 1}{" "}
                    {(bookingDetails.nights || 1) === 1 ? "night" : "nights"}
                  </div>
                  <div className="font-bold text-lg">
                    {formatPrice(convertPrice(adjustedTotal))}
                  </div>
                </div>
              </div>
            </CardContent>
                ...
                <div className="grid gap-8">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {availableRooms[0]?.maxAdult || 0}{" "}
                          {availableRooms[0]?.maxAdult === 1
                            ? "adult"
                            : "adults"}
                          {availableRooms[0]?.maxChild > 0 &&
                            ` and ${availableRooms[0].maxChild} ${
                              availableRooms[0].maxChild === 1
                                ? "child"
                                : "children"
                            }`}
                        </span>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid gap-2">
                      ...
                      <p className="mt-2">
                        Detailed check-in instructions will be provided after
                        booking.
                      </p>
                    </div>
                  </div>
                      ...
                      {availableRooms.length > 0 ? (
                            ...
                            >((acc, room) => {
                              ...
                              if (!acc[key]) {
                                acc[key] = {
                                  ...
                                  bedtype: room.bedtype || "Unknown",
                                  bedType:
                                    room.bedType || room.bedtype || "Unknown",
                                  size: room.size || room.roomsize || "Unknown",
                                  features: room.features ?? [],
                                };
                              }
                              if (
                                acc[key].mealPlans.every(
                                  (mp) => mp.mealplan !== room.mealPlan
                                )
                              ) {
                              ...
                              <RoomCard
                                ...
                                showQuantitySelector={
                                  ...
                                  bookingDetails.selectedRooms.some(
                                    (r) => Number(r.roomId) === Number(room.id)
                                  )
                                }
                                onMealPlanChange={(newRate: number) =>
                                  handleMealPlanChange(room.id, newRate)
                                }
                              />
                            ))}
                        </div>
                        ...
                        <div className="p-8 text-center border rounded-md">
                          <p className="text-muted-foreground mb-2">
                            No rooms available for the selected dates and
                            guests.
                          </p>
                          <p>
                            Please try different dates or adjust your guest
                            count.
                          </p>
                        </div>
                      )}
                      ...
                      {packages.length > 0 ? (
                        <div className="grid gap-6">
                          {packages.map((pkg) => (
                            <div
                              key={pkg.PackageID}
                              className="flex border rounded-lg overflow-hidden shadow-md bg-background hover:shadow-lg transition group"
                            >
                              <img
                                src={pkg.ImageURL}
                                alt={pkg.Description}
                              ...
                              <div className="flex flex-col justify-between p-4 flex-1">
                                <div>
                                  <div className="text-lg font-bold text-black mb-1 group-hover:underline">
                                    {pkg.PackageName}
                                  </div>
                                ...
                                <div className="flex items-center justify-between mt-auto">
                                  ...
                                  {selectedPackages.filter(
                                    (sel) => sel.PackageID === pkg.PackageID
                                  ).length === 0 ? (
                                    <Button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                      ...
                                      className="bg-black text-white px-2 py-1 rounded hover:bg-gray-800 text-sm"
                                    >
                                      Add to Booking
                                    </Button>
                                  ) : (
                    ...
                    {bookingDetails.selectedRooms.length > 0 && (
                      <div className="pt-2 mt-2 ">
                        ...
                        {bookingDetails.selectedRooms.map((room) => (
                          <div
                            key={room.roomId}
                            className="flex justify-between items-center p-3 border rounded-md mb-1"
                          >
                            <div>
                              <div className="font-medium">{room.roomName}</div>
                              <div className="text-sm text-muted-foreground">
                    ...
                    <>
                      {discountValue > 0 && (
                        <div className="flex justify-between items-center pt-1 text-sm text-green-600">
                          <span>Promo ({promoDetails?.PromoCode ?? ""})</span>
                          <span>
                            -{formatPrice(convertPrice(discountValue))}
                          </span>
                        </div>
                      )}
                      <div className="pt-4 mt-4 border-t">
      ...
      <div className="fixed bottom-20 right-6 z-50" style={{ zIndex: 9999 }}>
        <a
