...
import { Baby, CheckCircle, Download, X } from "lucide-react";
...
export default function BookingTEntativePage({
  params,
}: {
  params: { refno: string };
}) {
      ...
      try {
        ...
        if (data?.length > 0) {
          ...
          if (booking.Status === "CONFIRMED") {
            router.replace(`/confirmation/${params.refno}`);
  ...
  console.log("Hotel Data: ", hotelData || "not found");
    ...
    <div className="container max-w-2xl mx-auto py-12 px-4">
      ...
      <Card className="border border-green-500 shadow-lg">
        ...
        <CardHeader className="text-center space-y-3">
          <CheckCircle className="w-12 h-12 text-yellow-400 mx-auto" />
          <CardTitle className="text-2xl font-bold">
            Tentative Reservation
          </CardTitle>
          <p className="text-muted-foreground">
            Thanks <strong>{booking.BookerFullName}</strong>, your booking at{" "}
            <strong>{booking.HotelName}</strong> is tentatively secured, and we
            kindly request you to complete the payment by{" "}
            {booking.ReleaseDate ? (
              <>
                <strong>
                  {format(new Date(booking.ReleaseDate), "yyyy, MMM dd")}
                </strong>{" "}
                at{" "}
                <strong>
                  {format(new Date(booking.ReleaseDate), "hh:mm a")}
                </strong>
              </>
            ) : (
              "-"
        ...
        <CardContent className="space-y-6">
          {booking.IBE_Header && (
            <div className="bg-gray-50 border rounded-md p-4 text-center font-normal text-base text-black">
              {booking.IBE_Header}
            </div>
          )}
        ...
        <div style={{ pageBreakInside: "avoid" }}>
          <Card className="border border-green-500">
            ...
            >
              ...
              <p className="text-muted-foreground">
                Thanks <strong>{booking.BookerFullName}</strong>, your booking
                is <strong>{booking.HotelName}</strong> tentatively secured.
              </p>
              <p className="text-sm text-muted-foreground mt-2">
                Confirmation #: <strong>{booking.ReservationNo}</strong>
              </p>
    ...
    <div className="border rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 p-4 space-y-4 shadow-inner">
      ...
      {booking.IBE_Taxes && (
        <div className="flex items-start gap-2 text-blue-800">
          ...
          <div className="text-sm">
            <span className="font-semibold">Child Policy:</span>{" "}
            {booking.IBE_ChildPolicy}
          </div>
        </div>
      )}
      ...
      {booking.IBE_Taxes && (
        <div className="flex items-start gap-2 text-blue-800">
          ...
          <div className="text-sm">
            ...
            {booking.IBE_CancellationPolicy}
          </div>
        </div>
      )}
