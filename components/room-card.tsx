...
export function RoomCard({
  ...
  const fetchFeatures = useCallback(async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/API_IBE/GetRoomFeatures.aspx?roomtypeid=${room.id}`
      );
  ...
  const handleGuestChange = (
    ...
    const selectedPlan = room.mealPlans.find(
      (mp) => mp.mealplan === selectedMealPlan
    );
    let rate = 0;
    ...
    if (selectedPlan) {
      ...
      const childRate =
        typeof selectedPlan.childrate === "number" ? selectedPlan.childrate : 0;
  ...
  const handleMealPlanChange = (mealPlanId: string) => {
    ...
    if (selectedPlan) {
      ...
      const baseRate = (() => {
        switch (totalAdults) {
          case 1:
            return selectedPlan.singlerate ?? selectedPlan.doublerate ?? 0;
          case 2:
            return selectedPlan.doublerate ?? 0;
          case 3:
            return selectedPlan.triplerate ?? selectedPlan.doublerate ?? 0;
          case 4:
            return selectedPlan.qdplrate ?? selectedPlan.doublerate ?? 0;
          case 5:
            return selectedPlan.familyerate ?? selectedPlan.doublerate ?? 0;
          default:
            return selectedPlan.familyerate ?? selectedPlan.doublerate ?? 0;
        }
      })();
  ...
  const currentPlan = room.mealPlans.find(
    (mp) => mp.mealplan === selectedMealPlan
  );
  ...
  const minimumStayNotMet =
    typeof room.minstay === "number" &&
    room.minstay > 0 &&
    nights < room.minstay;
  console.log(
    "CheckIn:",
        ...
        <div className="grid md:grid-cols-5 gap-4">
          <div className="md:col-span-2 relative h-48 md:h-full">
            <div onClick={() => setShowModal(true)} className="cursor-pointer">
              <Image
                src={
                  room.mainimageurl || "/placeholder.svg?height=300&width=500"
                }
                alt={room.name}
                fill
          ...
          <div className="p-4 md:col-span-3 flex flex-col">
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {typeof room.minstay === "number" && room.minstay > 0 && (
                    ...
                    >
                      Minimum stay: {room.minstay} night
                      {room.minstay > 1 ? "s" : ""}
                    </p>
                  )}
                </div>
                <div className="text-right space-y-0.5">
                  ...
                  {/* This is your fetched per-night rate multiplied by nights */}
                  <div className="text-xs text-muted-foreground">
                    (
                    {formatPrice(
                      convertPrice(
                        (selectedRoom?.price ?? localPrice ?? room.price) /
                          (bookingDetails.nights || 1)
                      )
                    )}{" "}
                    = per night × {bookingDetails.nights || 1})
                  </div>
              ...
              <div className="flex flex-wrap gap-4 my-3 text-xs text-blue-700">
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    ...
                    {room.maxChild > 0 &&
                      ` and ${room.maxChild} ${
                        room.maxChild === 1 ? "child" : "children"
                      }`}
                  </span>
                </div>
                  ...
                  <ul className="text-sm text-muted-foreground mb-2 list-disc list-inside">
                    {room.mealPlans.map((plan) => (
                      <li key={plan.mealplan}>
                        {plan.mealplandesc} -{" "}
                        {formatPrice(convertPrice(plan.doublerate))}/period
                      </li>
                    ))}
                  </ul>
                  <div className="mb-2">
                ...
                <div className="flex justify-end sm:justify-start">
                  {minimumStayNotMet ? (
                    <div className="text-xs text-red-600 font-medium">
                      This room requires a minimum stay of {room.minstay} night
                      {room.minstay > 1 ? "s" : ""}.
                    </div>
                    ...
                    <Button onClick={handleSelectRoom} className="btn-dynamic">
                      Add to Booking
                    </Button>
                  )}
                </div>
        ...
        {showModal && (
          ...
          >
            ...
            >
              ...
              <Image
                src={
                  room.mainimageurl || "/placeholder.svg?height=800&width=1200"
                }
                alt={room.name}
                width={1200}
                height={800}
